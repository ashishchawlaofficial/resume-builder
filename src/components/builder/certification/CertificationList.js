...
import { deleteCertificationRecord } from "../../../store/slices/certificationSlice";
import DeleteModal from "../../common/layout/DeleteModal";

const CertificationList = ({ listItem, index, requestUrlChunk }) => {
  const dispatch = useDispatch();
  const [modalOpened, setModalOpened] = useState(false);

  const handleDelete = (id) => {
    dispatch(deleteCertificationRecord(requestUrlChunk, id));
        ...
        <Paper p="md" withBorder mb={20}>
          <Flex position="apart">
            ...
            <Box sx={{ width: "85%" }}>
              ...
              <Text c="dimmed" fz="sm">
                {listItem.authority} ({startDate} - {endDate})
              </Text>
