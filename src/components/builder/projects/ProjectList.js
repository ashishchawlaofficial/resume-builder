...
import { deleteProjectsRecord } from "../../../store/slices/projectsSlice";
import DeleteModal from "../../common/layout/DeleteModal";

const ProjectList = ({ listItem, index, requestUrlChunk }) => {
  const dispatch = useDispatch();
  const [modalOpened, setModalOpened] = useState(false);

  const handleDelete = (id) => {
    dispatch(deleteProjectsRecord(requestUrlChunk, id));
        ...
        <Paper p="md" withBorder mb={20}>
          <Flex position="apart">
            ...
            <Box sx={{ width: "85%" }}>
              ...
              <Text c="dimmed" fz="sm">
                {listItem?.projectUrl} ({startDate} - {endDate})
              </Text>
